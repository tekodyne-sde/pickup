...
# Pose pipeline (pose.py) — optional, GUI degrades gracefully if deps missing
try:
    from pose import (GraspPoseEstimator, mask_from_obb, save_annotated_snapshot,
                      show_open3d_view, CLASS_NAMES, DEPTH_SCALE,
                      SUCTION_PATCH_RADIUS, MODEL_PATH, INFER_EVERY_N_FRAMES)
    from ultralytics import YOLO
    HAS_POSE = True
except ImportError as _e:
    HAS_POSE = False
    POSE_IMPORT_ERROR = str(_e)
...
class CameraThread(threading.Thread):
    def __init__(self, app):
        ...
        self.active = True
        self.latest = {
            "rgb": None, "depth_raw": None, "depth_vis": None, "fps": 0.0, "det": None
        }
        self.intrinsics = None  # (fx, fy, cx, cy) at FRAME_WIDTH x FRAME_HEIGHT, set on connect
    ...
    def _oakd_loop(self):
        ...
        with dai.Device(maxUsbSpeed=dai.UsbSpeed.HIGH) as device:
            ...
            # RGB intrinsics at the GUI resolution (needed by the pose pipeline)
            try:
                calib = device.readCalibration()
                intr = calib.getCameraIntrinsics(
                    dai.CameraBoardSocket.CAM_A,
                    resizeWidth=FRAME_WIDTH, resizeHeight=FRAME_HEIGHT)
                self.intrinsics = (intr[0][0], intr[1][1], intr[0][2], intr[1][2])
                self.app.log(f"Intrinsics: fx={intr[0][0]:.1f} fy={intr[1][1]:.1f}")
            except Exception as e:
                self.app.log(f"Could not read intrinsics: {e}")

            model = None
            if HAS_POSE:
                model_file = Path(__file__).parent / MODEL_PATH
                if model_file.exists():
                    self.app.log("Loading YOLO model (best.pt)...")
                    model = YOLO(str(model_file))
                    self.app.log("YOLO model ready.")
                else:
                    self.app.log(f"Model not found: {model_file}")
            else:
                self.app.log(f"Pose deps missing ({POSE_IMPORT_ERROR}). pip install ultralytics open3d scipy")
            ...
            while self.active:
                ...
                if in_rgb is not None and in_depth is not None:
                    ...
                    # Live YOLO-OBB detection (every Nth frame, keep last result between runs)
                    frame_i += 1
                    det = self.latest["det"]
                    if model is not None and self.app.detect_enabled and frame_i % INFER_EVERY_N_FRAMES == 0:
                        results = model(rgb_frame, verbose=False)
                        obb = results[0].obb
                        if obb is not None and len(obb.cls) > 0:
                            i = int(np.argmax(obb.conf.cpu().numpy()))
                            det = {
                                "corners": obb.xyxyxyxy[i].cpu().numpy().reshape(4, 2),
                                "class_name": CLASS_NAMES.get(int(obb.cls[i]), str(int(obb.cls[i]))),
...
class CaptureApp(ctk.CTk):
    def __init__(self):
        ...
        # Pose pipeline state
        self.detect_enabled = False          # plain bool: read from camera thread
        self.pose_busy = False
        self.opt_detect = ctk.BooleanVar(value=False)
        self.opt_show_3d = ctk.BooleanVar(value=True)
    ...
    def _build_sidebar(self):
        ...
        ctk.CTkLabel(p, text="POSE", font=("", 11, "bold"), text_color=C_MUTED).pack(anchor="w", padx=12, pady=(14,2))
        ctk.CTkSwitch(p, text="Live Detection (YOLO)", variable=self.opt_detect,
                      command=self._toggle_detect).pack(anchor="w", padx=12, pady=(4, 0))
        ctk.CTkSwitch(p, text="Open 3D Viewer on Pose", variable=self.opt_show_3d).pack(anchor="w", padx=12, pady=(8, 0))
        if not HAS_POSE:
            ctk.CTkLabel(p, text="pose deps missing:\npip install ultralytics open3d scipy",
                         font=("Consolas", 9), text_color=C_WARN, justify="left").pack(anchor="w", padx=12, pady=(6, 0))

    def _build_center(self):
        p = self.center
        
        self.cam_scroll = ctk.CTkScrollableFrame(p, fg_color=C_BG, corner_radius=0)
        self.cam_scroll.pack(fill="both", expand=True)

        self.cam_label = ctk.CTkLabel(self.cam_scroll, text="", fg_color=C_CARD, corner_radius=0)
        ...
        self.btn_pose = ctk.CTkButton(ctrl, text="◎ POSE CAPTURE", width=140, height=44, fg_color=C_ACCENT2, font=("", 14, "bold"), command=self._pose_capture)
        self.btn_pose.pack(side="left", padx=4, pady=10)
    ...
    def _build_right(self):
        ...
        ctk.CTkLabel(p, text="POSE RESULT", font=("Consolas", 9), text_color=C_MUTED).pack(anchor="w", padx=10, pady=(4,2))
        self.pose_box = ctk.CTkTextbox(p, height=150, fg_color=C_CARD, border_color=C_BORDER, border_width=1, font=("Consolas", 10), text_color=C_ACCENT2)
        self.pose_box.pack(fill="x", padx=8, pady=(0,8))
        self.pose_box.insert("end", "No pose captured yet.")
        self.pose_box.configure(state="disabled")
    ...
    def _render_frame(self):
        ...
        # Detection overlay (corners are in full-frame coords)
        det = data.get("det")
        if det is not None:
            cv2.polylines(rgb, [det["corners"].astype(np.int32)], True, (0, 255, 0), 3)
            cv2.putText(rgb, f'{det["class_name"]} {det["conf"]:.2f}',
                        tuple(det["corners"][0].astype(int)),
                        cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 0), 2)

        # Up and down view at 720p resolution to allow detailed scrolling
        rgb_small = cv2.resize(rgb, (1280, 720))
        depth_small = cv2.resize(depth, (1280, 720))
        combined = np.vstack((rgb_small, depth_small))

        if time.time() < self.flash_until:
            overlay = combined.copy()
            cv2.rectangle(overlay, (0,0), (combined.shape[1], combined.shape[0]), (255,255,255), -1)
            cv2.addWeighted(overlay, 0.2, combined, 0.8, 0, combined)

        # Draw FPS and Crosshair over combined
        fps = data.get("fps", 0)
        cv2.putText(combined, f"FPS: {fps}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,255,0), 2)
        
        # Scale to perfectly fit the width, so horizontal scrolling isn't needed
        ww = self.cam_scroll.winfo_width() - 25 # 25px for vertical scrollbar
        if ww < 10:
            ww = 640
        ...
        frame = cv2.resize(combined, (new_w, new_h))
        frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)

        pil = Image.fromarray(frame_rgb)
        ctkimg = ctk.CTkImage(pil, size=(new_w, new_h))
        self.cam_label.configure(image=ctkimg)
        self.cam_label._image = ctkimg
    ...
    def _toggle_detect(self):
        self.detect_enabled = self.opt_detect.get()
        self.log(f"Live detection {'ON' if self.detect_enabled else 'OFF'}")

    def _pose_capture(self):
        if not HAS_POSE:
            self.log(f"Pose unavailable: {POSE_IMPORT_ERROR}. pip install ultralytics open3d scipy")
            return
        if self.pose_busy:
            self.log("Pose estimation already running...")
            return
        data = self.cam_thread.get_latest()
        rgb, depth_raw, det = data.get("rgb"), data.get("depth_raw"), data.get("det")
        intr = self.cam_thread.intrinsics
        if rgb is None or depth_raw is None:
            self.log("No frame available.")
            return
        if intr is None:
            self.log("No camera intrinsics (demo mode?) — pose unavailable.")
            return
        if det is None:
            self.log("No detection. Enable 'Live Detection (YOLO)' and aim at an object.")
            return
        self.pose_busy = True
        self.btn_pose.configure(state="disabled", text="◎ COMPUTING...")
        self.log(f"Pose capture: {det['class_name']} (conf={det['conf']:.2f})")
        threading.Thread(target=self._pose_worker, args=(rgb, depth_raw, det, intr), daemon=True).start()

    def _pose_worker(self, rgb, depth_raw, det, intr):
        try:
            fx, fy, cx, cy = intr
            estimator = GraspPoseEstimator(fx, fy, cx, cy, depth_scale=DEPTH_SCALE)
            mask = mask_from_obb(det["corners"], rgb.shape)
            pose, pcd = estimator.estimate(depth_raw, mask, det["corners"],
                                           patch_radius=SUCTION_PATCH_RADIUS)

            if pose is None:
                self.log("=> NO VALID GRASP FOUND")
                self._set_pose_text(f"{det['class_name']} (conf={det['conf']:.2f})\nNO VALID GRASP FOUND")
            else:
                p, n, q = pose['position'], pose['normal'], pose['quaternion']
                txt = (f"{det['class_name']} (conf={det['conf']:.2f})\n"
                       f"Position (m):\n  x={p[0]:.4f} y={p[1]:.4f} z={p[2]:.4f}\n"
                       f"Normal:\n  {n[0]:.3f}, {n[1]:.3f}, {n[2]:.3f}\n"
                       f"Quat (x,y,z,w):\n  {q[0]:.3f}, {q[1]:.3f}, {q[2]:.3f}, {q[3]:.3f}\n"
                       f"Flatness: {pose['flatness_score']:.5f}\n"
                       f"Inliers: {pose['inlier_count']}")
                self.log("=> GRASP POSE FOUND")
                self._set_pose_text(txt)

            out_dir = Path(self.opt_output_dir.get()) / self.opt_variant_name.get() / "pose"
            out_dir.mkdir(parents=True, exist_ok=True)
            out_path = out_dir / f"pose_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg"
            save_annotated_snapshot(rgb, det["corners"], det["class_name"],
                                    pose, fx, fy, cx, cy, str(out_path))
            self.log(f"Saved pose snapshot -> {out_path.name}")

            if self.opt_show_3d.get() and len(pcd.points) > 0:
                show_open3d_view(pcd, pose)  # blocking, but we're on a worker thread
        except Exception as e:
            self.log(f"Pose estimation error: {e}")
        finally:
            self.pose_busy = False
            self.after(0, lambda: self.btn_pose.configure(state="normal", text="◎ POSE CAPTURE"))

    def _set_pose_text(self, txt):
        def _apply():
            self.pose_box.configure(state="normal")
            self.pose_box.delete("1.0", "end")
            self.pose_box.insert("end", txt)
            self.pose_box.configure(state="disabled")
        self.after(0, _apply)
